...
for d in */.git; do
    ...
    clear
    readarray -t fnames < <(find "$(dirname "${d}")" -type f 2> /dev/null | grep -Fv -e doc/ -e openflights/ -e .git/ -e validator/ -e build/ -e .png -e .bak -e .bin -e .jpg -e .pyc -e .o -e .mod -e .pdf -e .ppm -e .pgm -e .pbm -e .csv -e .zip -e DS_Store | sort)
