...
mypy="/opt/local/bin/mypy-3.13"
pylint="/opt/local/bin/pylint-3.13"
...
# Loop over Git directories and Git files ...
for g in $(find -X . -name .git 2> /dev/null | sort | cut -c 3-); do
    # Deduce directory ...
    d="$(dirname "${g}")"
